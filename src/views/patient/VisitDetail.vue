        ...
        <v-row class="mb-4" no-gutters>
          ...
          <v-col cols="12" md="4" sm="6">
            <v-list-item two-line>
              <v-list-item-content>
                <v-list-item-title class="font-weight-bold">{{ visitDetail.details.doctor }}</v-list-item-title>
                <v-list-item-subtitle><small>نام پزشک</small></v-list-item-subtitle>
              </v-list-item-content>
            </v-list-item>
          </v-col>
          <!--تاریخ نوبت -->
...
export default {
  ...
  created() {
    ...
    this.$store.getters.api({
      method: 'get',
      url: `api/v1/dashboard/patient/visits/${id}`,
      data: false,
      loader: true
    }).then((r) => {
      this.visitDetail = {
        details: {
          ...
          doctor: `${r.data.doctor_first_name} ${r.data.doctor_last_name}`,
